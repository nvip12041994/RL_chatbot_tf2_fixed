...
class PolicyGradient_chatbot():
    ...
    def build_model(self):
        word_vectors = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.n_encode_lstm_step, self.dim_wordvec])

        caption = tf.compat.v1.placeholder(tf.int32, [self.batch_size, self.n_decode_lstm_step+1])
        caption_mask = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.n_decode_lstm_step+1])

        word_vectors_flat = tf.reshape(word_vectors, [-1, self.dim_wordvec])
        wordvec_emb = tf.compat.v1.nn.xw_plus_b(word_vectors_flat, self.encode_vector_W, self.encode_vector_b ) # (batch_size*n_encode_lstm_step, dim_hidden)
        wordvec_emb = tf.reshape(wordvec_emb, [self.batch_size, self.n_encode_lstm_step, self.dim_hidden])

        reward = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.n_decode_lstm_step])
        ...
        for i in range(0, self.n_decode_lstm_step):
            with tf.device("/cpu:0"):
                current_embed = tf.nn.embedding_lookup(params=self.Wemb, ids=caption[:, i])
            ...
            logit_words = tf.compat.v1.nn.xw_plus_b(output2, self.embed_word_W, self.embed_word_b)
            ...
            pg_current_loss = tf.reduce_sum(input_tensor=pg_cross_entropy) / self.batch_size
            pg_loss = pg_loss + pg_current_loss

        with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=tf.compat.v1.AUTO_REUSE):
            train_op = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(pg_loss)
    ...
    def build_generator(self):
        ...
        word_vectors_flat = tf.reshape(word_vectors, [-1, self.dim_wordvec])
        wordvec_emb = tf.compat.v1.nn.xw_plus_b(word_vectors_flat, self.encode_vector_W, self.encode_vector_b)
        wordvec_emb = tf.reshape(wordvec_emb, [self.batch_size, self.n_encode_lstm_step, self.dim_hidden])
        ...
        for i in range(0, self.n_decode_lstm_step):
            ...
            if i == 0:
                # <bos>
                with tf.device('/cpu:0'):
                    current_embed = tf.nn.embedding_lookup(params=self.Wemb, ids=tf.ones([self.batch_size], dtype=tf.int64))
            ...
            logit_words = tf.compat.v1.nn.xw_plus_b(output2, self.embed_word_W, self.embed_word_b)
            max_prob_index = tf.argmax(input=logit_words, axis=1)
