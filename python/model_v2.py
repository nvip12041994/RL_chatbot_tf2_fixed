...
class Seq2Seq_chatbot():
    ...
    def build_model(self):
        ...
        for i in range(0, self.n_decode_lstm_step):
            ...
            current_loss = tf.reduce_sum(input_tensor=cross_entropy)/self.batch_size
    ...
    def build_generator(self):
        word_vectors = tf.compat.v1.placeholder(tf.float32, [1, self.n_encode_lstm_step, self.dim_wordvec])
        ...
        for i in range(0, self.n_decode_lstm_step):
            ...
            if i == 0:
                with tf.device('/cpu:0'):
                    current_embed = tf.nn.embedding_lookup(params=self.Wemb, ids=tf.ones([1], dtype=tf.int64))
